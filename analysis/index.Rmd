...
draw_schedule <- function(
  ...
  people %>%
    ...
    mutate(
      ...
      month = fct(as.character(month), levels = month.abb[c(3:12, 1:2)]),
      year = year(utc)
    ) %>%
    ggplot(aes(month, member)) +
    geom_raster(aes(fill = status), alpha = 0.6) +
    geom_text(aes(label = time), size = 3) +
    facet_grid(rows = vars(tz), cols = vars(year), scales = "free", space = "free", ) +
...
1. Fixing the start time at 12pm UTC for every meeting
2. Ensure two regions are favoured
3. Placing the 'horrible times' in the empty space between the furthest spread members
4. Starting based on the Western-most member of each region

Key daylight savings shifts *occur in April* (South off + North on) and *Oct/Nov* (South On, North Off), with some staggering of commencement + exit dates.
*All proposals have been set to try and sync in with these shifts as smoothly as possible.*
...
people <- list(
  ...
  Zuguang = "Asia/Hong_Kong"
)
dates <- tibble(
  date = ymd("20260301") + seq(0, 365),
...
people %>% 
  ...
  mutate(
    ...
    future_utc = tz %>% 
      lapply(
        \(x) time_at_tz(now() + 3 * 30 * 24*60 * floor(365/4), x)
      ) %>% 
      map_chr(format, "%z")
  ) %>% 
  ...
  pander(
    justify = "llrlr",
    caption = "
    Breakdown of all time zones with the current +UTC shift and where that shift will be 3 months into the future.
...
## Proposal One: Fix Times at 12pm UTC

This effectively places the horrible meeting times between Australia/Melbourne and America/Chicago for the entire year.

Key Points:

- This is good for the key members Lori, Kevin & Stevie
- **Strongly disadvantages Mengbo** 
- Possibly workable but **also inconvenient for Zahraa** & all Asia/Pacific members
- Offers the highest number of meetings scheduled during business hours

```{r schedule-utc-fixed, fig.cap = "Proposed meeting times holding the start time fixed at 12pm UTC"}
p <- dates %>% 
  mutate(
    utc = as_datetime(date + 12/24)
  ) %>% 
...
## Proposal Two: Favouring Two Time Zones

This proposal sets the start time at 7am for the earliest member favouring:

1. The Americas, Europe & Africa (Mar-Oct)
2. Europe, Africa & the Asia-Pacific (Nov-Feb)

Key points:

- Those in the Americas will find it hard to attend Nov-Feb. *This includes Lori as a key member*
- Strongly disadvantages Zahraa & Mengbo, who never have meetings during business hours
...
p <- dates %>% 
  mutate(
    utc = case_when(
      ...
      # month %in% c("Sep", "Oct") ~ as_datetime(date + 6/24),
      # month %in% c("Apr") ~ as_datetime(date + 6/24),
      # month %in% c("Mar", "Apr") ~ as_datetime(date + 7/24),
...
## Proposal Three: Placing Difficult Times In The Empty Space
...
dates %>% 
  mutate(
    ...
    `Asia/Hong_Kong` = time_at_tz(date, "Asia/Hong_Kong")
  ) %>% 
  mutate(
    ...
    `Asia/Hong_Kong` = hour(`Asia/Hong_Kong`) + minute(`Asia/Hong_Kong`) / 60,
    pacific_diff = abs(`Australia/Melbourne` - `US/Central`),
    contintental_diff = abs(`Asia/Hong_Kong` - `Europe/Helsinki`)
  ) %>% 
...
Whilst previously, a viable gap existed between Kozo as the western-most member of the Asia-Pacific Region and Tuomas as the eastern-most member of the European region, with Zuguang's return to China, this no longer forms a viable option.

Key points:

- No-one has a meeting starting anywhere between 1am and 5am, however
    - Zuguang will have `r nrow(p@data %>% dplyr::filter(member == "Zuguang", time_dbl == 6))` meetings scheduled at 6am
    - Tuomas will have `r nrow(p@data %>% dplyr::filter(member == "Tuomas", time_dbl == 0))` meetings at midnight
    - Multiple Europeans have meetings after 11pm
    - Mengbo has `r nrow(p@data %>% dplyr::filter(member == "Mengbo", time_dbl == 23))` meeting(s) after 11pm
    - Zuguang & Kozo never have a meeting in business hours
...
p <- dates %>% 
  mutate(
    utc = case_when(
      month %in% c("Jan", "Feb", "Nov", "Dec") ~ as_datetime(date - 2 /24),
      TRUE ~ as_datetime(date - 12/24)
    ),
    year = year(utc)
  ) %>% 
...
## Proposal Four: Start Times Based On The Western-Most Member
...
We then start meeting times *at 6 or 7am for that member* to best accommodate other regions, and ensuring at least two larger regions have workable times.

1. 6am starts for Zuguang (Asia).
2. 7am starts for Zahraa (US).
3. 7am starts for Western Europe

- *Everyone gets at least 4 meetings in standard business hours (8am to 5pm as shown in green)*
...
p <- dates %>% 
  mutate(
    utc = case_when(
      ...
      month %in% c("Aug", "Sep", "Oct") ~ as_datetime(date + 6/24),
      month %in% c("Nov") ~ as_datetime(date + 7/24),
